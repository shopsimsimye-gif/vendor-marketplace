...
class Manager
{
    ...
    public static function set(string $key, $value, int $ttl = self::DEFAULT_TTL, string $group = self::GROUP): bool
    {
        ...
        // [QA 2026-08-02] تسجيل المفتاح ليتيح مسحه بالبادئة (deleteByPrefix)
        // عند تحديث البيانات — لازمه لمسح api_vendor_* بعد تحديث الملف الشخصي.
        self::add_key_to_group($key, $group);
     ...
     *
     * [QA 2026-08-02] يستخدم get_group_keys() — لذلك يجب أن تكون المفاتيح
     * قد سُجّلت عبر add_key_to_group() عند set().
     *
     * @param string $prefix بادئة المفتاح (مثل api_vendor_)
     * @param string $group مجموعة الكاش
     * @return int عدد المفاتيح المحذوفة
     */
    public static function deleteByPrefix(string $prefix, string $group = self::GROUP): int
    {
        $keys = self::get_group_keys($group);
        $deleted = 0;

        foreach ($keys as $key) {
            if (str_starts_with($key, $prefix) && self::delete($key, $group)) {
                $deleted++;
            }
        }
