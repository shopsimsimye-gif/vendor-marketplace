...
class MediaRepository implements MediaRepositoryInterface
{
    ...
    public function paginateByFolder(int $folderId, int $vendorId, int $page = 1, int $perPage = 20): array
    {
        ...
        $results = $wpdb->get_results(
            $wpdb->prepare(
                "SELECT * FROM {$this->table} WHERE vendor_id = %d AND folder_id = %d ORDER BY created_at DESC LIMIT %d OFFSET %d",
                $vendorId, $folderId, $perPage, $offset
            ),
            ARRAY_A
        );
        $total = (int) $wpdb->get_var(
            $wpdb->prepare("SELECT COUNT(*) FROM {$this->table} WHERE vendor_id = %d AND folder_id = %d", $vendorId, $folderId)
        );
        return [
            'data' => array_map([MediaDTO::class, 'fromArray'], $results),
            'total' => $total,
    ...
    {
        global $wpdb;
        return (bool) $wpdb->update($this->table, ['folder_id' => $folderId], ['id' => $mediaId], ['%d'], ['%d']);
    }
