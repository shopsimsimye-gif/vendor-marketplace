...
class WithdrawalService
{
    ...
    public function requestWithdrawal(int $vendorId, float $amount, string $method = 'bank_transfer', array $methodDetails = []): int
    {
        ...
        // [QA 2026-08-02] تعقيم method_details قبل التخزين — يمنع XSS مخزّن
        // عند عرض تفاصيل طلب السحب لاحقاً في لوحة التحكم (كل قيمة sanitize_text_field).
        $cleanMethodDetails = [];
        foreach ($methodDetails as $detailKey => $detailValue) {
            $cleanKey = sanitize_text_field((string) $detailKey);
            if (is_array($detailValue)) {
                $cleanValue = array_map('sanitize_text_field', $detailValue);
            } else {
                $cleanValue = sanitize_text_field((string) $detailValue);
            }
            $cleanMethodDetails[$cleanKey] = $cleanValue;
        }
        ...
        $withdrawalId = $this->withdrawalRepository->create([
            ...
            'method_details' => $cleanMethodDetails,
        ]);
