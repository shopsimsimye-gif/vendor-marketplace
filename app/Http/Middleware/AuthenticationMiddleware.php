...
{
    /**
     * [QA 2026-08-02] الرسالة في const واحد لتجنب التكرار بين handle() و __invoke().
     */
    private const UNAUTHORIZED_MESSAGE = 'يجب تسجيل الدخول أولاً للوصول إلى هذا المورد.';
    ...
    public function handle(\WP_REST_Request $request, callable $next): \WP_REST_Response|\WP_Error
    {
        if (!is_user_logged_in()) {
            return new \WP_Error(
                'unauthorized',
                __(self::UNAUTHORIZED_MESSAGE, 'vmp'),
                ['status' => 401]
            );
        }
